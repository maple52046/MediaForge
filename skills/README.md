...
## Agent and IDE Integration (Wrapper Convention)
...
Each agent or IDE adds a thin wrapper that *references* the canonical file
instead of duplicating it:

- **Codex**: `.agents/skills/<name>/SKILL.md` carries standard skill frontmatter
  (`name`, `description`) and points back to `skills/<name>/SKILL.md`. Codex
  discovers these repository skills automatically; invoke one explicitly with
  `$<name>` or let Codex match its description. Do not place skills under
  `.codex/skills/`: project `.codex/` is for trusted configuration, hooks, and
